...
namespace Datos
{
    public class DPartido
    {
        ...
        public string RegistrarFixture(List<Partidos> listaPartidos)
        {
            try
            {
                using (var context = new BDEFEntities())
                {
                    context.Partidos.AddRange(listaPartidos);
                    context.SaveChanges();
            ...
            {
                return "Error al generar fixture: " + ex.Message;
            }
        }
        ...
        public Partidos ObtenerPorId(int idPartido)
        {
            using (var context = new BDEFEntities())
            {
                return context.Partidos
                              .Include(p => p.Equipos)
                              .Include(p => p.Equipos1)
                              .FirstOrDefault(p => p.IdPartido == idPartido);
            }
        }
        ...
        public string Modificar(Partidos partido)
        {
            try
            {
                using (var context = new BDEFEntities())
                {
                    var existente = context.Partidos.Find(partido.IdPartido);
                    if (existente != null)
                    {
                        existente.Estado = partido.Estado;
                        existente.IdGanador = partido.IdGanador;
                        context.SaveChanges();
            ...
            {
                return "Error al modificar partido: " + ex.Message;
            }
        }
